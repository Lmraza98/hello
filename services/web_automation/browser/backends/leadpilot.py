...
DEFAULT_OPENCLAW_AI_SNAPSHOT_MAX_CHARS = max(
    ...
    min(
        120_000,
        int(os.getenv("BROWSER_LEADPILOT_AI_SNAPSHOT_MAX_CHARS", os.getenv("BROWSER_OPENCLAW_AI_SNAPSHOT_MAX_CHARS", "20000")) or "20000"),
    ),
)
...
class LeadPilotBackend(BrowserBackend):
    ...
    def _base_url(self) -> str:
        raw = (
            os.getenv("LEADPILOT_BROWSER_BASE_URL")
            or os.getenv("OPENCLAW_BROWSER_BASE_URL")
    ...
    def _auth_headers(self) -> dict[str, str]:
        ...
        token = (
            os.getenv("LEADPILOT_BROWSER_AUTH_TOKEN")
            or os.getenv("LEADPILOT_BROWSER_TOKEN")
            or os.getenv("OPENCLAW_BROWSER_AUTH_TOKEN")
            or os.getenv("OPENCLAW_BROWSER_TOKEN")
            or ""
        ).strip()
        password = (os.getenv("LEADPILOT_BROWSER_PASSWORD") or os.getenv("OPENCLAW_BROWSER_PASSWORD") or "").strip()
        if token:
            headers["Authorization"] = f"Bearer {token}"
        elif password:
            headers["x-leadpilot-password"] = password
        return headers
    ...
    async def _request(
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail={
                    "code": "leadpilot_unreachable",
                    "message": f"LeadPilot browser bridge is not reachable at {base}",
                },
            ) from exc
        except httpx.HTTPError as exc:
            raise HTTPException(status_code=502, detail={"code": "leadpilot_error", "message": str(exc)}) from exc
        ...
        if response.status_code >= 400:
            ...
            raise HTTPException(
                ...
                detail={
                    "code": "leadpilot_request_failed",
                    "message": "LeadPilot browser bridge request failed",
                    "details": detail,
                },
            )
    ...
    async def _refresh_tab_maps(self) -> list[dict[str, Any]]:
        ...
        #
        # LeadPilot's /tabs ordering can change (new tabs, anti-bot interstitials, etc.).
    ...
    async def health(self) -> dict[str, Any]:
        ...
        return {"ok": True, "mode": "leadpilot", "connected": running}
    ...
    async def tabs(self) -> dict[str, Any]:
        ...
        return {"tabs": tabs, "active_tab_id": active_tab_id, "mode": "leadpilot"}
    ...
    async def navigate(
        ...
        *,
        ...
        if not isinstance(out, dict):
            return {"ok": True, "tab_id": tab_id, "url": url, "mode": "leadpilot"}
    ...
    async def snapshot(self, *, tab_id: str | None = None, mode: str | None = None) -> dict[str, Any]:
        ...
        # LeadPilot supports multiple snapshot shapes under format=ai:
    ...
    async def find_ref(
        ...
        *,
        ...
        # LeadPilot pages can re-render frequently (SPA). Refs can be absent briefly after navigation
    ...
    async def act(
        ...
        *,
        ...
        def _extract_leadpilot_error(exc: HTTPException) -> str:
            try:
        ...
        if action_norm == "evaluate":
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": "evaluate is disabled in leadpilot mode. Use snapshot+act primitives and skills instead.",
                },
            )
        ...
        if action_norm == "click":
            ...
            try:
                ...
                err = _extract_leadpilot_error(exc)
    ...
    async def screenshot(self, *, tab_id: str | None = None, full_page: bool | None = None) -> dict[str, Any]:
        ...
        if not isinstance(out, dict) or not out.get("ok") or not out.get("path"):
            raise HTTPException(status_code=502, detail={"code": "screenshot_failed", "message": "LeadPilot screenshot failed"})
