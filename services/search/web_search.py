...
import config
from api.observability import record_cost
...
_TAVILY_ENABLED = os.getenv("TAVILY_ENABLED", "false").strip().lower() == "true"
...
async def tavily_search(
    ...
    if not _TAVILY_ENABLED:
        return _shape_error(query, "Tavily is disabled (set TAVILY_ENABLED=true to enable)")
    ...
    try:
        ...
        record_cost(
            ...
            feature=feature,
            endpoint=endpoint,
            usd=float(config.TAVILY_COST_PER_REQUEST_USD),
            meta={"query": query, "max_results": max_results, "search_depth": search_depth, "tool": "tavily"},
        )
