...
from services.salesforce_pages import (
...
class SalesforceBot:
    ...
    async def _try_autofill_login(self) -> bool:
        ...
        """
        try:
            from services.salesforce_credentials import get_credentials
                ...
                try:
                    ...
                    if await field.count() > 0:
                        if await field.is_visible():
                            ...
                            # Hidden field — check if it already contains a value
    ...
    async def find_lead_url_by_name(self, full_name: str) -> Optional[str]:
        ...
        # ── Fast path: grab the URL directly from the results table ──
        ...
        # ── Fallback: click through to the record ──
    ...
    async def process_send_item(self, item: Dict, review_mode: bool = False) -> Dict:
        ...
        try:
            ...
            # Parse contact name using particle-aware normalization
            full_name = item.get('contact_name', '')
            normalized = normalize_name(full_name)
            first_name = normalized.first if normalized.first else None
            last_name = normalized.last if normalized.last else None
            
            # Log if name needs review
            if normalized.needs_review:
                print(f"    [SFBot] ⚠️  Name may need review: {normalized.review_reason}")
