...
lives in markdown skills under `skills/` and is loaded via `services.browser_skills.store`.
...
from services.browser_skills.store import append_repair_note, get_action_hints, get_skill, match_skill
from services.browser_challenges import classify_challenge
...
from services.browser_stealth import (
    StealthConfig,
    action_delay,
    human_click,
    human_delay,
...
from services.browser_policy import AgentState, BrowserPolicy
...
class BrowserWorkflow:
    ...
    async def bind_skill(self, *, task: str, url: str | None = None, query: str | None = None) -> bool:
        """Bind this workflow to a matched skill (based on url/task/query)."""
        url = url or await self.current_url()
        matched = match_skill(url=url, task=task, query=query)
