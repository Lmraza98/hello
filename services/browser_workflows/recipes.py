...
from services.browser_skills.store import upsert_skill
...
def build_salesnav_account_search_url(*, keyword: str, filters: dict[str, Any]):
    ...
    from services.linkedin.salesnav.query_builder import build_salesnav_account_search_url as _build

    return _build(keyword=keyword, filters=filters)


def build_salesnav_people_search_url(*, keyword: str, filters: dict[str, Any]):
    from services.linkedin.salesnav.query_builder import build_salesnav_people_search_url as _build
...
def _decompose_salesnav_query(query: str) -> dict[str, Any] | None:
    ...
    try:
        from services.linkedin.salesnav.filter_parser import parse_salesnav_query
...
async def auto_learn_skill(
    ...
    *,
    ...
    """Auto-learn a browser skill by snapshotting the current page and using
    an LLM to infer the extraction pattern.

    Returns ``{"ok": True, "skill_id": ..., "draft": ...}`` on success,
    or ``{"ok": False, "draft": ..., "error": ...}`` on failure.  The draft
    is always included (when available) so the caller can present it to the
    user even if auto-save failed.
    """
    try:
        # 1. Navigate to a sensible page if not already there.
        # Infer the target site from the task name or query.
        ...
        if not needs_navigation:
            # Check if the current page is relevant to the task
        ...
        if needs_navigation:
            # Try to derive a URL from the task name
            # Common patterns: "youtube_video_stats" → youtube.com
            #                  "google_search" → google.com
            #                  "linkedin_*" → linkedin.com
            site_hints = {
            ...
            if not target_url:
                # Last resort: Google the query to land on a relevant page
                target_url = f"https://www.google.com/search?q={query.replace(' ', '+')}"

            # For sites with search, append the query to the URL
        ...
        # 2. Snapshot the page to get the accessibility tree
        refs = await wf.snapshot()

        if not refs:
            return {"ok": False, "error": "Page snapshot is empty — cannot learn from a blank page."}

        # 2. Build a compact representation of the page for the LLM
        # Include only the first 150 refs to keep the prompt manageable.
        compact_refs = []
        for ref in refs[:150]:
            entry = {}
            if ref.get("role"):
        ...
        # 5. Generate the SKILL.md content
        ...
        search_role = inferred.get("search_input_role") or "input"
        search_text = inferred.get("search_input_text") or "search"
        href_pattern = inferred.get("result_href_pattern") or ""
        ...
        try:
            ...
            return {"ok": True, "skill_id": skill_id, "draft": draft}
        except Exception as exc:
            logger.warning("Failed to save auto-learned skill: %s", exc)
            return {"ok": False, "skill_id": skill_id, "draft": draft, "error": str(exc)}
