...
async def enroll_and_draft(
    ...
    if contact_id is None and create_if_missing:
        contact_id = db.add_linkedin_contact(
            company_name=create_if_missing.get("company_name", ""),
            domain=create_if_missing.get("domain"),
            name=create_if_missing.get("name", ""),
            title=create_if_missing.get("title"),
