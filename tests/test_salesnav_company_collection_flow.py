...
def test_extract_company_focus_filters_generic_queries():
    flow = SalesNavCompanyCollectionFlow.__new__(SalesNavCompanyCollectionFlow)

    assert flow._extract_company_focus("find OpenAI on linkedin sales navigator") == "OpenAI"
    assert flow._extract_company_focus("healthcare companies in united states") is None
    assert flow._extract_company_focus("  ") is None
