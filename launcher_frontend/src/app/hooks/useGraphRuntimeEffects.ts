...
import { resolveScopedGraph } from "../utils/resolveScopedGraph";
...
type PathPlaybackStateLike = {
  completed: Set<string>;
  running: Set<string>;
  blocked: Set<string>;
  activeTransition?: { kind?: string; nodeId?: string; sourceEventId?: string } | null;
  transitionCursorUsed?: number;
  activeNodeId?: string;
  explanation?: string;
};

export type UseGraphRuntimeEffectsParams = {
  runtimeDebug: boolean;
  bridge: BridgeApi | null;
  tab: string;
  graphModel: GraphModelLike;
  graphState: GraphRuntimeState;
  setGraphState: Setter<GraphRuntimeState>;
  ...
  setStatusById: Setter<Record<string, StatusRowLike>>;
  ...
  setChildScopeEvents: Setter<RunEvent[]>;
  setChildProgressByParent: Setter<Record<string, ChildProgressRow[]>>;
  runAutoTrackRef: { current: { runId: string; aggregateId: string; engaged: boolean } };
  setGraphDetailsOpen: Setter<boolean>;
  graphNodesWithPlayback: GraphNodeLike[];
  artifactReplayMode: boolean;
  suppressLiveGraphAutotrack: boolean;
  playbackActiveNodeId: string;
  activeAggregateNode: GraphNodeLike | null;
  detailsAggregateNode: GraphNodeLike | null;
  graphActiveChildId: string;
  graphDetailChildId: string;
  childAttemptById: Record<string, string | number>;
  activeChildProgressRows: ChildProgressRow[];
  pathPlaybackState: PathPlaybackStateLike;
  activeRunRow: RunRow | null;
};

export function useGraphRuntimeEffects(params: UseGraphRuntimeEffectsParams) {
  const {
    runtimeDebug,
    bridge,
    tab,
  ...
  useEffect(() => {
    setGraphState((prev) => {
      ...
      const scoped = resolveScopedGraph(nextNodes, nextEdges, graphScopedModel);
      ...
      const pathEntry = mode === "path" ? (nextPathSteps[cursor] || null) : null;
      const timelineEntry = mode === "timeline" ? (nextEvents[cursor] || null) : null;
      const transitionCursor = mode === "path"
        ? Number(pathEntry?.transitionCursor ?? -1)
        : (timelineEntry ? cursor : -1);
      const activeNodeRaw = String(timelineEntry?.nodeId || "");
      const activeNodeId =
        String(graphScope?.level || "suite") === "suite"
  ...
  }, [graphModel, graphScopedModel, graphScope?.level, normalizeSuiteSelectionNodeId]);

  useEffect(() => {
    setGraphState((prev) => {
      const transitions = prev.playback?.transitions || [];
      const branchLockId = String(prev.playback?.branchLockId || "");
      const scoped = resolveScopedGraph(prev.nodes || [], prev.edges || [], graphScopedModel);
      const scopedNodes = scoped.nodes;
      const scopedEdges = scoped.edges;
      const nextPathSteps = buildPathSteps({
        nodes: scopedNodes,
  ...
  }, [graphScope, graphScopedModel, graphState.playback?.branchLockId]);
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      setGraphState((prev) => {
        const mode = prev.playback?.mode || "timeline";
        const pathEntries = prev.playback?.pathSteps || [];
        const timelineEntries = prev.events || [];
        const activeEntries = mode === "path" ? pathEntries : timelineEntries;
        ...
        const pathActive = mode === "path" ? (pathEntries[nextCursor] || null) : null;
        const timelineActive = mode === "timeline" ? (timelineEntries[nextCursor] || null) : null;
        const selectedEventId = mode === "path"
          ? (pathActive?.sourceEventId || prev.selectedEventId)
          : (timelineActive?.id || prev.selectedEventId);
        return {
          ...prev,
          playback: {
            ...
            transitionCursor: mode === "path" ? Number(pathActive?.transitionCursor ?? prev.playback?.transitionCursor ?? -1) : nextCursor,
            branchLockId: String(prev.playback?.branchLockId || ""),
          },
          selectedEventId,
          selectedNodeId: mode === "path"
            ...
            : (
                (String(graphScope?.level || "suite") === "suite"
                  ? normalizeSuiteSelectionNodeId(String(timelineActive?.nodeId || ""), prev.nodes || [])
                  : String(timelineActive?.nodeId || ""))
                || prev.selectedNodeId
              ),
        };
      });
  ...
  function updateGraphPlayback(patch: Partial<{ isPlaying: boolean; cursor: number; speed: number; mode: "timeline" | "path" }>) {
    setGraphState((prev) => {
      const nextMode = patch.mode == null ? (prev.playback?.mode || "timeline") : patch.mode;
      const pathEntries = prev.playback?.pathSteps || [];
      const timelineEntries = prev.events || [];
      const activeEntries = nextMode === "path" ? pathEntries : timelineEntries;
      const max = Math.max(0, activeEntries.length - 1);
      ...
      const pathActive = nextMode === "path" ? (pathEntries[nextCursor] || null) : null;
      const timelineActive = nextMode === "timeline" ? (timelineEntries[nextCursor] || null) : null;
      const selectedEventId =
        nextMode === "path" ? (pathActive?.sourceEventId || prev.selectedEventId) : (timelineActive?.id || prev.selectedEventId);
      const next = {
        ...prev,
        playback: {
          ...
          transitions: prev.playback?.transitions || [],
          pathSteps: prev.playback?.pathSteps || [],
          transitionCursor: nextMode === "path" ? Number(pathActive?.transitionCursor ?? prev.playback?.transitionCursor ?? -1) : nextCursor,
          branchLockId: String(prev.playback?.branchLockId || ""),
        },
      };
      if (patch.cursor != null || patch.mode != null) {
        const activeNodeId = String(pathActive?.focusNodeId || timelineActive?.nodeId || "");
