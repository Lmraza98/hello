...
import type { RunRow } from "../app/types/contracts";

type LastRunStripProps = {
  latestRun: RunRow | null;
  statusById: Record<string, { status?: string }>;
  onStopRun: () => void;
};

export default function LastRunStrip({ latestRun, statusById, onStopRun }: LastRunStripProps) {
  const statusRows = Object.values((statusById || {}) as Record<string, { status?: string }>);
  const total = statusRows.length;
  const done = statusRows.filter((s) => ["passed", "failed", "canceled", "timed_out"].includes(String(s.status || ""))).length;
  const running = statusRows.filter((s) => String(s.status || "") === "running").length;
  const queued = statusRows.filter((s) => String(s.status || "") === "queued").length;
