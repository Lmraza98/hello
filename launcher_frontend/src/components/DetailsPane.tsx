...
import type { GraphNodeLike } from "./graph/graphTypes";
import type { RunEvent } from "../lib/graph/types";
import type { RunRow, StatusRow, TestRow } from "../app/types/contracts";
import type { BridgeApi } from "../app/types/common";
...
type GraphContextLike = {
  active?: boolean;
  compactInspector?: boolean;
  selectedNode?: GraphNodeLike;
  events?: RunEvent[];
  screenshotsById?: Record<string, unknown>;
  ...
  onOpenRun?: (params: { runId?: string; nodeId?: string }) => void;
};

type RunInspectorLike = {
  selectedContext?: {
    runId?: string;
  ...
  modelIds?: {
    ...
    aggregateChildren?: Array<{ id?: string; raw?: string }>;
  };
  runData?: {
    testsCount?: number;
    testsSample?: Array<{ id?: string; status?: string; children?: Array<{ id?: string; status?: string }> }>;
  };
  eventSummary?: {
    total?: number;
    childEventsCount?: number;
    withDoubleColon?: number;
    equalsSelectedChild?: number;
    equalsRawChild?: number;
    lastEvents?: Array<{ ts?: string | number; type?: string; nodeId?: string; message?: string }>;
  };
  mismatchDetector?: {
  ...
  progressSummary?: {
    ...
    runningSample?: Array<{ childId?: string; status?: string }>;
  };
};
...
type DetailsPaneProps = {
  ...
  graphContext?: GraphContextLike | null;
  runInspector?: RunInspectorLike | null;
};
  ...
  const filteredChildren = useMemo(() => {
    ...
    if (q) {
      rows = rows.filter((row) =>
        `${row.name || ""} ${row.filePath || ""} ${row.id || ""} ${row.childGroup || ""}`.toLowerCase().includes(q)
      );
    }
    ...
    const groupRank = (row) => {
      const group = String(row?.childGroup || "").toLowerCase();
