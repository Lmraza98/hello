...
- Contacts details-panel resize handle now renders as a true 1px divider line with no surrounding visual gutter, while the invisible drag hit target is widened to make grabbing the divider easier.
- Contacts now measures the body scroll gutter and applies the same right-side compensation to the separate desktop header table, so opening the details pane no longer lets header column widths drift out of alignment with the row data.
- Documents now follows the same pinned right-edge `actions` column pattern as Contacts: the `Columns + arrows` controls live in the sticky actions header cell, and each desktop row gets a matching far-right ellipsis menu instead of the older floating header overlay.
- Documents now uses the same hidden-native-scrollbar plus overlay-thumb treatment as Contacts for the main desktop table body, so the sticky ellipsis/actions column is no longer inset by the native scrollbar gutter.
- Contact details activity table now gives spare panel width to `Activity` specifically instead of a separate blank filler slot, so the descriptive activity text expands to the right edge while `Type`, `Date`, and `Status` stay fully shown at their fitted support widths.
- Contact details activity table now resets to a fresh sizing baseline (`contact-activity-table-v4`) with larger default/min widths for `Type`, `Date`, and `Status`, so those support columns open fully by default while `Activity` remains the only flex column.
...
- Standard email list tables now use the same Contacts-style sticky right-edge actions column: the `Columns + arrows` controls render inside the actual actions header cell, and row actions open from a pinned far-right ellipsis menu instead of relying on the older floating overlay header.
- Campaign rows now move their operational controls into that ellipsis menu, replacing the previous inline action rail with a compact Contacts-style row-actions pattern while preserving campaign edit, upload, pause/activate, launch, view-contacts, and delete actions.
- Templates now use the shared email table path as well, with the same sticky actions header/cell pattern, hidden native scrollbar treatment, and a row ellipsis menu for open, duplicate, archive, and copy-export actions.
- Email and Templates toolbars now own the fixed-width `Columns + arrows` control block on the far right of the search row, while the corresponding sticky table header cell has been repurposed as a page-actions ellipsis slot for actions like new campaign, review approvals, scheduled processing, import JSON, and new template.
- Email review/scheduled/history details now use the same flatter Templates-style side-panel shell too: compact top metadata pills, a denser sticky action strip, flush subject/body sections, and no separate bottom `Details` accordion.
- Email details sequence history also now renders as the same compact shared-table pattern used in campaign details, with a real sticky header, aligned columns, and an overlay scrollbar instead of the older stacked list.
- Dashboard lower workspace cards (`Active Conversations`, `Today's Contacts`, and `Scheduled Sends`) now use tighter shared dashboard chrome too: compact 31px headers, reduced padding, denser list rows, and a cleaner mobile stack without the older oversized empty-state/card spacing.
- Dashboard now opts out of the sticky workspace header and shifts its lower three-card strip to the horizontal layout at `lg` instead of `xl`, so medium/desktop widths keep those cards side-by-side without the previous oversized vertical stacking.
- Dashboard top summary area is denser on smaller screens too: stat cards now stay in a 2x2 compact grid with reduced padding/icon weight, and the email-performance header/KPI strip uses tighter spacing so those sections no longer consume disproportionate height for the amount of information shown.
- The dashboard email-performance body is now horizontally flush as well: its inner wrapper no longer adds side padding, and the KPI strip no longer uses inter-cell gap spacing.
- Dashboard email-performance charts now preserve their aspect ratio inside the KPI card instead of stretching to fill arbitrary heights, and the trend lines/dots use lighter stroke weights so the graph reads cleaner at compact sizes.
- Dashboard charts now also measure their actual container width with `ResizeObserver` and render against that live width, so the graph stays aligned to the visible card width instead of drifting based on a fixed internal SVG width.
- Campaign details now renders enrolled contacts in a shared compact table with `Name`, `Email`, `Step`, `Date & Time`, and a right-edge ellipsis actions cell for contact removal instead of the older stacked scrolling list, and its scroll area now uses the same hidden-native-scrollbar plus overlay-thumb treatment as Contacts so the scrollbar no longer consumes table width.
- Campaign details no longer renders the old `Available to add` block beneath enrolled contacts; the panel is now focused on the enrolled table plus campaign metadata.
- Campaign details also no longer uses the old bottom `Details` accordion; cadence and enrolled-count metadata now live inline with the top status pills instead.
- Shared table header filters now support right-aligned popovers for trailing columns, and the campaign-details enrolled table uses that on its rightmost filter so the menu opens inward instead of running off-screen.
- Campaign-details enrolled contacts now display user-facing sequence labels instead of the internal zero-based `current_step`: pending contacts show `Step 1..N`, and contacts at or beyond the campaign length render as `Finished`.
- Campaign details now splits its lower pane into two equal shared-table sections: enrolled contacts on top and linked campaign templates below, both using the same compact table chrome, utility row, header ellipsis, column controls, and overlay scrollbar treatment.
- The Templates details/editor panel now follows the same compact side-panel language as campaign details: sticky top metadata, flatter action strips, reduced card rounding, denser field spacing, and section blocks that match the rest of the workspace instead of the older rounded editor slab.
- The Templates editor panel also drops the old `From email`, `Reply-to`, and `Live Preview` sections, and now places the variable-token block above the HTML body editor so the full editor fits within the side-panel height more cleanly.
- Templates panel utility sections now use flush section chrome with minimal internal padding, so variables, warnings, and blocks take less vertical space while staying visually aligned with the flatter campaign-details panel style.
- Documents now uses that same toolbar-slot pattern too: the far-right search-row utility block owns `Columns + arrows`, and the sticky table `actions` header cell is now reserved for page-level document actions like upload, new folder, tree-order toggle, and refresh.
- Contacts now follows the same toolbar-slot pattern as the other workspace tables: the fixed-width `Columns + arrows` cluster lives on the far right of the search row, and the sticky right-edge table `actions` header cell is now a page-actions ellipsis that currently exposes `New contact`.
- Tasks (the Browser workflow page) now follows that same Contacts table structure too: the toolbar owns the fixed-width `Columns + arrows` slot, the desktop table has a pinned right-edge actions column, and the sticky `actions` header cell is now a page-actions ellipsis instead of the older floating viewport overlay.
- Tasks also now moves its coarse filtering into the table itself: `Status` and `Source` use Contacts-style header filters, the old toolbar badges/checkbox strip is gone, and the remaining global `Show finished` toggle lives in the page-actions ellipsis instead of crowding the search row.
- Browser Workbench's `Browser Tabs` mode now uses the same shared fitted-table structure as Contacts as well: the tab list moved off its hand-built plain table and onto the shared header/colgroup path with a pinned right-edge actions column, toolbar-mounted `Columns + arrows`, and a page-actions ellipsis in the sticky header cell.
- Tabbed workspace pages now anchor their tab strip at the very top of the page shell: `WorkspacePageShell` no longer adds the old top padding when a `preHeader` exists, and the page-specific negative-margin compensation was removed from Contacts, Documents, Email, Templates, Tasks, and Browser Workbench.
