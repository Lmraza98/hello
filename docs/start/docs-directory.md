---
summary: "Curated links to the most used docs pages in this repository."
read_when:
  - You want the shortest path to key docs
  - You are debugging ingestion, API routes, or UI compatibility
title: "Docs Directory"
---

# Docs Directory

## Start Here

- [Documentation home](/)
- [System architecture](/concepts/architecture)
- [Browser automation](/concepts/browser-automation)
- [Workflow builder plan](/concepts/workflow-builder-plan)
- [Workflow builder master plan](/concepts/workflow-builder-master-plan)
- [Chat model routing](/concepts/chat-model-routing)
- [Tool planner tiering](/concepts/tool-planner-tiering)
- [API endpoint catalog](/api/endpoints)

## Data Layer

- [Sales Navigator automation](/SALESNAV_AUTOMATION)
- [Hybrid search](/hybrid_search)

## Engineering

- [Refactoring notes](/REFACTORING_NOTES)
- [UI compatibility](/UI_COMPATIBILITY)
- [FunctionGemma fine-tune workflow](/FUNCTIONGEMMA_FINETUNE)
- [Documentation workflow](/help/documentation-workflow)
- [Workflow Builder E2E testing](/help/workflow-builder-e2e-testing)
- [Launcher test orchestration (hub)](/help/launcher-test-orchestration)
- [Launcher test operator guide](/help/launcher-test-operator-guide)
- [Launcher test developer contract](/help/launcher-test-developer-contract)
- [Dashboard email performance](/help/dashboard-email-performance)
- [Email templates](/help/email-templates)
- [Context recovery](/help/context-recovery)
- [Repository map](/reference/repository-map)
